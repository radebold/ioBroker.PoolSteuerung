...
class Poolsteuerung extends utils.Adapter {
  ...
  async ensureState(id, type, role, def, write) {
  ...
  async onReady() {
    ...
    await this.ensureState('status.ph.previewMl', 'number', 'value', 0, false);
    await this.ensureState('status.ph.previewRuntimeSec', 'number', 'value', 0, false);
    await this.ensureState('status.ph.lastAction', 'string', 'text', '', false);
    await this.ensureState('status.chlorinator.lastAction', 'string', 'text', '', false);
    await this.ensureState('status.pump.lastAction', 'string', 'text', '', false);
    await this.ensureState('status.heatpump.lastAction', 'string', 'text', '', false);
    await this.ensureState('status.heatpump.lastReason', 'string', 'text', '', false);
    await this.ensureState('status.heatpump.waterTemp', 'number', 'value.temperature', 0, false);
    await this.ensureState('status.heatpump.energySource', 'string', 'text', '', false);
    ...
    await this.ensureState('control.manualDose', 'boolean', 'button', false, true);
    await this.setStateAsync('info.connection', true, true);
    this.subscribeStates('control.manualDose');
    this.debug('poolsteuerung 0.1.4 started');
  ...
  async setActorState(id, val, context) {
    if (!id) return;
    if (this.config.simulateMode) { this.debug(`${context}: simulate ${id}=${val}`); await this.setStateAsync('status.debug.lastDecision', `${context}: simulate ${id}=${val}`, true); return; }
  ...
  async setForeignValue(id, val, context) {
    if (!id) return;
    if (this.config.simulateMode) { this.debug(`${context}: simulate ${id}=${val}`); return; }
    await this.setForeignStateAsync(id, val);
  }
  ...
  async phPreview() {
    const ph = await this.getForeignNumber(this.config.phStateId); if (ph === null) return;
    const r = this.calcPhDose(ph);
    this.debug(`[PH] ph=${ph} delta=${r.delta} rawMl=${r.rawMl} limitedMl=${r.limitedMl} sec=${r.sec}`);
  ...
  async phDose(reason) {
    const ph = await this.getForeignNumber(this.config.phStateId); if (ph === null) return;
    const rel = await this.getForeignBoolean(this.config.phDoseEnableStateId, true); if (!rel) { this.debug('[PH] Freigabe fehlt'); return; }
    const pump = await this.getForeignBoolean(this.config.circulationPumpSocketStateId, false); if (!pump && this.config.phOnlyWhenPumpRunning) { this.debug('[PH] Pumpe aus'); return; }
    const r = this.calcPhDose(ph); if (r.limitedMl <= 0 || r.sec <= 0) return;
    this.phRunning = true;
    await this.setActorState(this.config.phPumpSocketStateId, true, 'PH_START');
    await this.setStateAsync('status.ph.lastAction', `${reason}: gestartet (${r.limitedMl} ml)`, true);
    setTimeout(async () => {
      await this.setActorState(this.config.phPumpSocketStateId, false, 'PH_ENDE');
      this.phRunning = false;
  ...
  async chlorTick() {
    ...
    const orp = await this.getForeignNumber(this.config.orpStateId); if (orp === null) return;
    ...
    this.debug(`[ORP] orp=${orp} state=${state} pump=${pump}`);
    if (this.config.chlorinatorOnlyWhenPumpRunning && !pump) { if (state) await this.setActorState(this.config.chlorinatorSocketStateId, false, 'CHLOR_AUS_PUMPE'); return; }
    const now = Date.now(); const minOn = (Number(this.config.chlorinatorMinOnMin) || 0) * 60000; const minOff = (Number(this.config.chlorinatorMinOffMin) || 0) * 60000;
    if (state && orp >= Number(this.config.orpOff || 780) && now - this.lastChlorSwitch >= minOn) {
      await this.setActorState(this.config.chlorinatorSocketStateId, false, 'CHLOR_AUS');
      ...
      await this.setActorState(this.config.chlorinatorSocketStateId, true, 'CHLOR_EIN');
      this.lastChlorSwitch = now;
  ...
  async pumpTick() {
    ...
    const now = this.hhmm(); const start = this.config.pumpStartTime || ''; const end = this.config.pumpEndTime || ''; if (!start || !end) return;
    const shouldRun = now >= start && now < end; const state = await this.getForeignBoolean(this.config.circulationPumpSocketStateId, false);
    this.debug(`[PUMP] now=${now} start=${start} end=${end} shouldRun=${shouldRun} state=${state}`);
    if (shouldRun && !state) { await this.setActorState(this.config.circulationPumpSocketStateId, true, 'PUMPE_EIN'); await this.setStateAsync('status.pump.lastAction', `Pumpe EIN ${now}`, true); }
    if (!shouldRun && state) { await this.setActorState(this.config.circulationPumpSocketStateId, false, 'PUMPE_AUS'); await this.setStateAsync('status.pump.lastAction', `Pumpe AUS ${now}`, true); }
  }
  async heatpumpOff(reason) {
    ...
    await this.setActorState(this.config.heatpumpPowerStateId, false, 'HEATPUMP_AUS');
  ...
  async heatpumpTick() {
    ...
    this.debug(`[HEAT] water=${waterTemp} feedIn=${feedIn} soc=${batterySoc} pump=${pump} power=${powerState}`);
    ...
    const target = Number(this.config.heatpumpTargetTemp || 24); const tempNeedsHeat = waterTemp < target;
    const batteryFull = batterySoc !== null && batterySoc >= Number(this.config.heatpumpBatteryFullSoc || 98);
    const byPv = feedIn !== null && feedIn >= Number(this.config.heatpumpPvMinFeedInW || 1500);
    const bySoc = batterySoc !== null && batterySoc >= Number(this.config.heatpumpMinBatterySoc || 95);
    let energyAllowed = false; let energySource = '';
    ...
    const now = Date.now(); const minOn = (Number(this.config.heatpumpMinOnMin) || 0) * 60000; const minOff = (Number(this.config.heatpumpMinOffMin) || 0) * 60000;
    if (!tempNeedsHeat) { if (powerState && now - this.lastHeatpumpSwitch < minOn) return; await this.heatpumpOff('Zieltemperatur erreicht'); return; }
    if (!energyAllowed) { if (powerState && now - this.lastHeatpumpSwitch < minOn) return; await this.heatpumpOff('Keine Energie-Freigabe'); return; }
    if (!powerState && now - this.lastHeatpumpSwitch < minOff) return;
    if (!powerState) {
      await this.setForeignValue(this.config.heatpumpModeStateId, 1, 'HEATPUMP_MODE');
      await this.setForeignValue(this.config.heatpumpSetTempStateId, target, 'HEATPUMP_SETTEMP');
      await this.setActorState(this.config.heatpumpPowerStateId, true, 'HEATPUMP_EIN');
